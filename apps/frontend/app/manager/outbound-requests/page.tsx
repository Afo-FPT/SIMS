...
export default function ManagerOutboundRequestsPage() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const requests = await listStorageRequests({ requestType: 'OUT' });
      setItems(requests);
      setPage(1);
    } catch (e) {
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          message="No outbound requests have been created yet."
        />
      ) : filteredItems.length === 0 ? (
        <EmptyState
          icon="search_off"
          title="No results found"
          message="No outbound requests match your current filters. Try adjusting or clearing them."
        />
      ) : (
