...
export default function StaffInboundRequestsPage() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await listStorageRequests({ requestType: 'IN', status: 'APPROVED' });
      setItems(data);
    } catch (e) {
            ...
            <TableBody>
              {items.map((r) => (
                <TableRow key={r.request_id}>
                  ...
                  <TableCell>
                    <Link href={`/staff/inbound-requests/${r.request_id}?from=inbound`} className="text-sm font-bold text-primary hover:underline">
                      Putaway
                    </Link>
                  </TableCell>
                </TableRow>
